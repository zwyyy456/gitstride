...
Use `bundle exec ruby fastlane/exclude_china_mainland.rb` to configure App Store availability through the API. For an app without an availability record, it selects every current territory except **China mainland**, disables pre-orders, and leaves future new territories unavailable. For an existing record, it only disables China mainland. The script verifies China mainland is unavailable afterward. The binary, App Review login, age rating, encryption declaration, pricing, and remaining compliance questions are separate release inputs.
