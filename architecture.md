...
- Issue 创建由 `ProjectStore` 创建和更新 `IssueCreation` 操作，View 只持有只读引用。操作在内存中绑定原连接与原项目，保留已确认身份、未完成字段和下一阶段；恢复只继续未完成步骤。仓库与指派人解析、缺失标签创建属于 Issue 提交前的准备阶段，其失败不得标记为 Issue 创建结果不确定。已经取得身份或 Issue 提交结果不确定时，不得重新执行创建请求。GitHub 确认 Project membership 和请求字段后，Store 直接提交新条目并结束前台创建；后续读取在观察到同一条目及已确认字段前必须保留该结果，全量补刷不阻塞创建完成。操作引用释放后不提供跨启动恢复。
